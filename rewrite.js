...
export default function Comments({ novelId }: { novelId: string }) {
  ...
  const submitEdit = (id: string, parentId?: string) => {
    ...
    const updated = comments.map(c => {
      if (parentId) {
        if (c.id === parentId && c.replies) {
          c.replies = c.replies.map(r => r.id === id ? { ...r, text: editText } : r);
        }
      } else {
        if (c.id === id) {
  ...
  const renderCommentBody = (item: any, isReply = false, parentId?: string) => {
    ...
    const isMyComment = item.userName === currentUserName || item.userName === "??? ???????"; // Example logic to allow edits based on username

    return (
      <div className="flex-1 w-full">
        <div className="flex justify-between items-start mb-2">
          <span className="font-bold text-gray-300">{item.userName} {isReply && <span className="text-xs text-gray-600 font-normal">??</span>}</span>
        ...
        {isEditing ? (
          <div className="mt-2 text-right">
            <textarea
              ...
              className="w-full bg-gray-900/80 border border-gray-700 text-gray-200 rounded-lg p-3 focus:border-red-900 outline-none h-20"
            ></textarea>
        ...
        {!isEditing && (
          <div className="flex justify-end gap-3 mt-3">
             {!isReply && (
               <button onClick={() => { setReplyingToId(item.id); setEditingId(null); }} className="text-xs text-blue-500 hover:text-blue-400 font-bold transition-colors">
                  ?? ?
               </button>
             )}
             {isMyComment && (
  ...
  return (
    <div className="mt-12 bg-black/40 border border-red-900/30 p-6 rounded-2xl shadow-xl w-full max-w-4xl mx-auto backdrop-blur-sm" dir="rtl">
      <h3 className="text-2xl font-bold text-red-500 mb-6 flex items-center gap-2">
        <span>?????????</span>
        <span className="text-sm bg-red-900/50 px-2 py-1 rounded-full text-white/80">{comments.length + comments.reduce((acc, c) => acc + (c.replies?.length || 0), 0)}</span>
      </h3>
      ...
      <form onSubmit={handlePostComment} className="mb-8 border-b border-gray-800/50 pb-8">
        ...
        <button
          ...
          className="mt-3 bg-red-700 hover:bg-red-800 text-white px-6 py-2 rounded-lg font-bold transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
        >
          ??? ???????
        </button>
      </form>
          ...
          comments.map((comment) => (
            <div key={comment.id} className="bg-gray-900/20 p-5 rounded-xl border border-gray-800/60 shadow-md">
              ...
              {replyingToId === comment.id && (
                <div className="mt-4 ml-0 md:mr-8 border-t border-gray-800/50 pt-4 flex flex-col items-end">
                   <textarea
                    value={replyText}
              ...
              {comment.replies && comment.replies.length > 0 && (
                <div className="mt-4 mr-4 md:mr-10 border-r-2 border-gray-700/50 pr-4 space-y-4">
                  {comment.replies.map(reply => (
                    <div key={reply.id} className="bg-gray-800/30 p-4 rounded-lg border border-gray-700/30">
