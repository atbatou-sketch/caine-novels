...
const fileBuffer = Buffer.from(`"use client";
...
export default function Comments({ novelId }: { novelId: string }) {
  ...
  const fetchComments = () => {
    try {
      const dbComments = JSON.parse(localStorage.getItem(\`caine-comments-\${novelId}\`) || "[]");
  ...
  const saveToStorage = (updatedComments: CommentNode[]) => {
    localStorage.setItem(\`caine-comments-\${novelId}\`, JSON.stringify(updatedComments));
  ...
  const renderCommentBody = (item: any, isReply = false, parentId?: string) => {
    ...
    // Allow edit/delete if it's the user's comment, or if they are admin
    const isAdmin = currentUserName === "كين العبقري" || currentUserName === "مدير";
    const isMyComment = item.userName === currentUserName || isAdmin;

    // The user name object check for backward compatibility with old comments format
      ...
      <div className="flex-1 w-full" dir="rtl">
        <div className="flex justify-between items-start mb-2">
          <span className="font-bold text-gray-300 gap-2 flex items-center">
             ...
             {isAdmin && item.userName === currentUserName && <span className="text-xs text-red-500 bg-red-900/30 px-2 py-0.5 rounded font-bold">أدمن</span>}
          </span>
        ...
        {!isEditing && (
          <div className="flex justify-end gap-3 mt-3 border-t border-gray-800/40 pt-2">
             ...
             {isMyComment && (
                <>
                  <button onClick={() => startEdit(item.id, item.text)} className="text-sm text-green-500 hover:text-green-400 font-bold transition-colors">
                    تعديل ✎
                  </button>
                  <button onClick={() => handleDelete(item.id, parentId)} className="text-sm text-red-500 hover:text-red-400 font-bold transition-colors">
                    مسح 🗑
                  </button>
                </>
             )}
  ...
  return (
    <div className="mt-12 bg-black/40 border border-red-900/30 p-6 rounded-2xl shadow-xl w-full max-w-4xl mx-auto backdrop-blur-sm" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        ...
        {comments.length > 0 && (currentUserName === "كين العبقري" || currentUserName === "مدير") && (
            <button onClick={handleClearAll} className="text-xs bg-red-900/40 text-red-400 hover:bg-red-800 hover:text-white px-3 py-1.5 rounded-lg transition-colors border border-red-900/50 font-bold">
      ...
      <div className="space-y-6">
        {loading ? (
          ...
          comments.map((comment) => (
            <div key={comment.id} className="bg-gray-900/40 p-5 rounded-xl border border-gray-800/60 shadow-md">
              ...
              {replyingToId === comment.id && (
                <div className="mt-4 border-t border-gray-800/50 pt-4 flex flex-col items-end mr-0 md:mr-8">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setReplyingToId(null)} className="text-sm px-4 py-1.5 rounded-lg bg-gray-800 text-white hover:bg-gray-700">إلغاء</button>
                    <button onClick={() => submitReply(comment.id)} disabled={!replyText.trim()} className="text-sm px-4 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50">إرسال الرد</button>
                  </div>
                </div>
              )}

              {comment.replies && comment.replies.length > 0 && (
                <div className="mt-4 mr-4 md:mr-10 border-r-2 border-red-900/30 pr-4 space-y-4">
                  {comment.replies.map(reply => (
                    <div key={reply.id} className="bg-gray-800/30 p-4 rounded-lg border border-gray-700/40 shadow-sm relative before:absolute before:content-[''] before:w-4 before:h-px before:bg-red-900/30 before:right-[-18px] before:top-6">
          ...
          <p className="text-center text-gray-600 italic py-8">لا توجد تعليقات بعد... كن أول من يكتب.</p>
        )}
      </div>
    </div>
  );
...
fs.writeFileSync('src/components/Comments.tsx', fileBuffer);
